...
Rollback uses the retained previous pair but targets **every scope selected by this rotation**, including Dependabot when the previous lifecycle mentioned only Actions. Each rollback delivery is journaled separately and can itself resume after interruption. The previous certificate must still be valid and its matching private key available. No previous key can be retrieved from either GitHub scope.
