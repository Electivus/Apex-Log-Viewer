...
Before `grant-runtime`, native `prove` or the explicit license fallback can proceed, assignment inventory must contain only the known runtime set, recorded owned-ECA preauthorization sets and Salesforce's generated set for the user's already verified minimum profile. Validation binds assignments to the assignee, live permission-set ID, recorded IDs where present, and the exact profile or app ownership contract. It rejects unknown sets even when all administrative flags are false, permission-set-group assignments, duplicates, mismatched IDs and incomplete inventories without removing access. Each present ECA preauthorization set also passes the complete authorization-only grant audit. Initial setup permits the stage before any ECA exists; proof additionally requires the runtime assignment to be present. The fallback removes only the runtime assignment returned by this audit.

A directed native read-only audit on 7 September 2026 at 15:55 UTC passed for the current identity's three assignments: runtime, Salesforce-generated minimum-profile set, and permanent ECA preauthorization. It preserved the private state and issued no mutation commands. The generated profile set is recognized by its actual `ProfileId` and type, not its generated name.
...
Before every proof, current Salesforce inventory must identify the owned ECA and its exclusive preauthorization set. A global assignment query must report completion and a matching total count, with exactly the recorded assignment to the dedicated user. Current OAuth policies are retrieved into a new private `app-<mode>/proof-audit-*` project, preventing partial retrieval from reusing older files. The existing policy/certificate verifier checks the controls and the returned client ID must match the private JWT inputs. Missing, changed or incomplete evidence stops before JWT login or remote mutation; no policy, grant, assignment or credential is repaired to pass this audit. The private audit metadata remains with the app's recovery files.
...
The maintenance read-back must report `LeaseState__c: disabled`, `HealthState__c: needs_recreate` and no scratch auth URL. Missing or different health fails at `pool-maintenance`, leaves the proof incomplete and still reconciles its owned resources through the existing cleanup path.
...
Proof intent starts in `initializing` with `remoteResourcesAttempted: false`, before directory creation and login. The marker is saved as true immediately before the first remote resource write. `cleanup-proof` can close a verified pre-resource attempt without CLI authentication, including an absent directory or partially prepared homes. Legacy `login` intent is also recoverable. Every current and historical proof must have a recognized phase before recovery, a new proof, revocation or private-file cleanup. Unknown, missing or invalid phases are rejected even when persisted cleanup flags are true or authenticated inventories would be empty; no earlier ledger entry is changed first. Preserve the state and runtime access for operator reconciliation instead of editing the phase into an apparently safe value. Contradictory resource IDs, attempted writes, missing authentication after an attempt and incomplete inventories likewise never establish cleanup. This releases later proof/revocation gates only when no resource write was attempted or owned deletion was reconciled.

A signup marked `Active` with no observable `ActiveScratchOrg` remains pending: no scratch cleanup is attested and revocation stays blocked. Retry `cleanup-proof` with the retained credentials when the owned scratch becomes visible. Successful deletion responses are recorded per signup and owner, allowing recovery after a later pool-cleanup failure even if the signup still reports `Active`. A lost deletion response is not such a receipt; absent a visible scratch, recovery waits for authoritative terminal signup evidence. An empty query alone never supplies that evidence for an `Active` signup.
...
The recognized runtime set is also re-audited from current Salesforce data before `prove`, an assigned-user `grant-runtime` rerun or the explicit fallback. The source permission-set metadata defines the exact allowed object, field, Apex and user permissions; all described permission flags and grant relationships must match, including absence of unrelated setup entities, custom permissions and other access. Extra, missing, duplicate or incompletely reported grants stop the command before login or mutation. Initial setup verifies the deployed set before assigning it and verifies the resulting assignment afterward. The persisted `runtime` summary records a previous observation and is never sufficient evidence for the next command.

The directed native read-only audit passed at **17:13:52 UTC on 7 September 2026** for the existing identity and all three legitimate assignments. It checked 1,334 permission flags, four object grants, 36 field grants, the two expected Apex bindings and the other described grant relations. Its 29 CLI commands performed no mutation, and the private identity state remained byte-for-byte unchanged. This audit verifies current grants; it does not repeat or replace the earlier native scratch/pool lifecycle proof.

The directed ECA audit passed at **22:43:27 UTC on 7 September 2026** using the same pre-proof verifier and native Salesforce CLI. It confirmed the owned app, complete exclusive assignment, current OAuth controls/certificate, matching private client ID and recognized proof phases. Its 15 read-only CLI commands performed zero remote mutations; the identity ledger, JWT inputs, certificate and private key remained byte-for-byte unchanged. Fresh retrieved metadata is retained privately under `app-permanent/proof-audit-*`, with sanitized evidence in `eca-policy-phase-audit-result.json`. The invoking Codex process needed the already-persisted user settings `NODE_USE_SYSTEM_CA` and `NODE_EXTRA_CA_CERTS` loaded into its environment; no persistent configuration was changed and TLS verification stayed enabled.
...
The later restricted grant-drift/cleanup correction passed **59 public-command tests** (including 17 effective-grant scenarios). The new cases reject changed grants before deployment/login/fallback, preserve initial setup and the legitimate profile/ECA assignments, keep unobservable Active signups pending, recover confirmed scratch deletion after a pool failure, and retain uncertainty after a lost deletion response. Both previously failing recovery fixtures use the same native real-path resolver as the command; their Windows capitalization regression was reproduced and corrected separately. Previously passing broad suites were not rerun for these isolated script changes.

The restricted ECA-policy/phase correction validated **111 public-command tests**: the full run passed 110 and exposed one fixture whose broad `proof-*` directory match also intercepted the new audit directory. After narrowing that fixture to the recorded proof directory, the exact failing test passed; production code did not change after the full run. New cases reject changed/missing policies, incomplete or nonexclusive assignments, identity drift and unknown current/historical phases before login or mutation, while retaining legitimate proof and recovery behavior. Targeted regressions failed before their fixes; canonical lint, formatting and diff checks passed. Unchanged passing suites were not repeated.

The subsequent maintenance assertion passed all **10 affected public-command scenarios**, including successful proof, missing/divergent health and owned cleanup/recovery. The health regression failed before correction. This adds a check on an already queried field; native commands, payloads and grants did not change, and no new native scratch/pool proof was run for this assertion.
