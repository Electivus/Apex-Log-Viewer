...
The initial replacement [#1112](https://github.com/Electivus/Apex-Log-Viewer/pull/1112)
merged as `470e616aae5696560b7798729f591c1e87d53e75`. All 25 original PRs
are closed: 23 superseded by that merge, and two deferred with reproduced
incompatibilities. All 20 remote checks passed; its PR description records
transient CI failures and retries. A separate authenticated proxy run passed
all seven local UI tests without retries.
...
## Follow-up generated by the new configuration

The configuration change generated six more PRs during the final inventory:

| PR                                                             | Update                                              | Disposition / evidence                                                                                                                                                                                                    |
| -------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [1113](https://github.com/Electivus/Apex-Log-Viewer/pull/1113) | Autoprefixer 10.6.0                                 | Include in a follow-up with a targeted lockfile update. Preserve Lucide 1.46.0 already resolved on main; the bot's unrelated downgrade to 1.45.0 is omitted.                                                              |
| [1114](https://github.com/Electivus/Apex-Log-Viewer/pull/1114) | Kotlin 2.4.20 / Gradle 9.7.1                        | Include with the missing dependency verification metadata. Its exact head `a69443360ee5326f5c1af65ad8184cc554c1d200` fails locally and in Linux/Windows CI because the Kotlin plugin marker POM has no approved checksum. |
| [1115](https://github.com/Electivus/Apex-Log-Viewer/pull/1115) | typescript-eslint 8.70.0                            | Include parser and plugin together, retaining the compatible TypeScript 6.0.3 compiler. The TypeScript 7 deferral does not exclude these compatible parser updates.                                                       |
| [1116](https://github.com/Electivus/Apex-Log-Viewer/pull/1116) | oclif runtime 4.14.0                                | Include within the supported oclif 4 family used by sf-plugins-core 12.                                                                                                                                                   |
| [1117](https://github.com/Electivus/Apex-Log-Viewer/pull/1117) | JSforce 3.10.25 / YAML 2.9.1 / tailwind-merge 3.7.0 | Include the four manifest updates and advance the workspace YAML override to 2.9.1 so the requested runtime update actually takes effect.                                                                                 |
| [1118](https://github.com/Electivus/Apex-Log-Viewer/pull/1118) | Node types 24.13.4                                  | Include the patch while retaining the Node 24 type family.                                                                                                                                                                |

The follow-up adds 60 SHA-256 entries, retaining every previous checksum and
the existing verification configuration. The hashes were independently compared
with Maven Central artifacts or published checksum files; the Kotlin JVM plugin
marker was compared with the Gradle Plugin Portal artifact, which is the
repository used by plugin resolution. The Portal and Maven Central publish
different POM text for that marker, so only the Portal's matching hash is added.

The Gradle 9.7.1 wrapper JAR and distribution hash match the official
[wrapper checksum](https://services.gradle.org/distributions/gradle-9.7.1-wrapper.jar.sha256)
and [distribution checksum](https://services.gradle.org/distributions/gradle-9.7.1-bin.zip.sha256).
Verification metadata was generated using Gradle's documented
[incremental update command](https://docs.gradle.org/current/userguide/dependency_verification.html#sec:updating-verification-metadata),
then checked by the ordinary strict build. The new local `.kotlin/` compiler
cache is ignored alongside the other IntelliJ build outputs.

The original #1114 real-org failures on Linux and Windows report the same
missing Kotlin marker checksum as the ordinary build jobs. Other original heads
show proxy UI readiness timeouts (#1113/#1115), Azure federated-token retrieval
failure (#1118), and zero ingested telemetry events (#1116). These failures
remain explicit validation targets for the combined replacement; they do not
justify relaxing checks or asserting that all original PRs were green.
