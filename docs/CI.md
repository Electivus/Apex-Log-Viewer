...
- The release workflow now runs `node scripts/verify-runtime-compatibility.mjs` against the packaged `linux-x64` artifact before upload/publish, and the extension release/pre-release workflows re-run that verifier against the fetched `linux-x64` binary before VSIX packaging so incompatible runtime bundles fail in CI instead of surfacing only in remote IDEs.
...
Concurrency: Workflows use concurrency groups to avoid duplicate runs per ref, except the real-org Playwright workflow when `SF_SCRATCH_POOL_NAME` is configured. That workflow queues by scratch-org pool name with `cancel-in-progress: false` so dependency bursts do not over-lease the shared Dev Hub pool.
...
- The workflow-level concurrency lock uses the configured `SF_SCRATCH_POOL_NAME` when pool mode is active, so all PRs sharing the same Dev Hub pool queue behind one another instead of exhausting pool leases during dependency bursts.
