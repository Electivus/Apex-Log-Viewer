...
async function resolveRipgrepBinary(): Promise<string> {
  ...
  if (cachedBinary === undefined) {
    ...
    candidates.push(path.resolve(__dirname, '..', 'bin', 'ripgrep', target, 'bin', binaryName));
    candidates.push('rg');
...
function processJsonLine(line: string, matches: RipgrepMatch[], cwd: string): void {
  ...
  try {
    ...
    const submatches = submatchesRaw
      .map(item => {
        const sm = item as { start?: unknown; end?: unknown };
        return { start: Number(sm.start), end: Number(sm.end) };
      })
